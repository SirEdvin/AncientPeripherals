...
public class SmithingAutomataCorePeripheral extends ExperienceAutomataCorePeripheral {
    ...
    protected int getMaxFuelConsumptionRate() {
        return ProgressivePeripheralsConfig.smithingAutomataCoreMaxFuelConsumptionLevel;
    }

    @Override
    protected int getRawCooldown(String s) {
        if (s.equals(SMELT_OPERATION))
            return ProgressivePeripheralsConfig.smeltCooldown;
        if (s.equals(SMITH_OPERATION))
            return ProgressivePeripheralsConfig.smithCooldown;
        return 0;
    }
    ...
    public Map<String, Object> getPeripheralConfiguration() {
        ...
        data.put("smithCost", ProgressivePeripheralsConfig.smithCost);
        data.put("smithCooldown", ProgressivePeripheralsConfig.smithCooldown);
        data.put("smeltCooldown", ProgressivePeripheralsConfig.smeltCooldown);
        data.put("fuelCostRate", ProgressivePeripheralsConfig.furnaceBurnFuelCostRate);
        return data;
    }
    ...
    public final MethodResult smith(int secondSlot, int targetSlot) throws LuaException {
        ...
        checkResults = consumeFuelOp(ProgressivePeripheralsConfig.smithCost);
        if (checkResults.isPresent()) return checkResults.get();
    ...
    public final MethodResult smelt(@Nonnull IArguments arguments) throws LuaException {
        ...
        int requiredFuelCount = (int) (ProgressivePeripheralsConfig.furnaceBurnFuelCostRate * DEFAULT_SMELT_OPERATION_COST * smeltCount * _getFuelConsumptionRate());
        World world = getWorld();
