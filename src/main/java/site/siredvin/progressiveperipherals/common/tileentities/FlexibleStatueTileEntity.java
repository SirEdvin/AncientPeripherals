...
public class FlexibleStatueTileEntity extends TileEntity {
    ...
    public void load(BlockState state, CompoundNBT tag) {
        ...
        if (tag.contains(BAKED_QUADS_TAG)) {
            bakedQuads = NBTUtils.readQuadList(tag.getByteArray(BAKED_QUADS_TAG));
            refreshShape();
        }
    }
    ...
    public void setBakedQuads(@Nonnull byte[] data) {
        QuadList quadList = NBTUtils.readQuadList(data);
        if (quadList != null)
            setBakedQuads(quadList);
    }
